...
We RMLD as a reversible perturbation by appealing to the theory of Markov generators. Define $T_t^0$ to be the Markov semigroup on $L^2(\pi)$ of LD. note that $T_t^0 f(x) = \mathbb{E}[f(X_t)|X_0 = x]$. The generator of the semigroup is a linear operator acting on functions in $L^2(\pi)$
\begin{align}
  \mathcal{L}_0 f = \text{lim}_{t\to 0}\frac{T_t^0 f - f}{t}, 
\end{align}
...
\begin{align}
  \left\| T_t^0 f - \int f\, d\pi \right\| \le Ce^{-\lambda_0 t} \left \| f- \int f \, d\pi \right\|
\end{align}
...
\begin{proof}
  ...
  \begin{align\*}
    \frac{d}{dt} \|T_t f\|^2 = 2\langle T_t f, (\mathcal{L}_0 + \mathcal{S}) T_t f\rangle = 2\langle T_t f, (\mathcal{L}_0 + \mathcal{S}) T_t f \rangle \le 2 \langle T_t f, \mathcal{L}_0T_t f\rangle \le -2 \lambda_0 \|T_tf\|^2. 
  \end{align\*}
  This implies that $\|T_t f\| \le e^{-\lambda_0t} \|f\|$, and therefore that $\text{Re}(\langle ( \mathcal{L} + \lambda_0) f, f\rangle) \le 0$. Therefore, the spectrum of $T_t$ is such that $\Re(z) \le -\lambda_0$, which mean the spectral gap is smaller.  
\end{proof}
