...
<h3> Proof  </h3>
  ...
  \end{align\*}
  This implies that $\left\lVert T\_t f\right\rVert \le e^{-\lambda\_0t} \left\lVert f \left\lVert$, and therefore that $\text{Re}(\langle ( \mathcal{L} + \lambda\_0) f, f\rangle) \le 0$. Therefore, the spectrum of $T\_t$ is such that $\text{Re}(z) \le -\lambda\_0$, which mean the spectral gap is smaller.  
---
...
\begin{align\*}
 ...
 \end{align}
 But this statement is trivial since $\mathcal{R}$ is positive semidefinite. (As an aside, this is true by the spectral theorem for compact self-adjoint operators and the functional calculus.) This shows that as long as $\mathcal{S}$ is a reversible perturbation that is negative semidefinite, the asymptotic variance of the ergodic average estimator will decrease. The rest of the paper in \cite{reybellet2016} derives many large deviations principles for these reversibly (and irreversibly) perturbed systems, and relates the asymptotic variance with the derived rate functions. They also formally show conditions that are required of the perturbations to see strict improvement. (Namely, they show that $A(x)-I$ needs to be positive semidefinite for a reversible perturbation to lead to improvements). 
